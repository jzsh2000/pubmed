...
mkdir -p $output_dir/{publication-date,mesh-term,publication-type,content,date,author}
...
find $input_dir -name '*.gz' \
    | parallel -j1 --verbose "$script_dir/extract-author.sh {} > $output_dir/author/{/.}.author"
